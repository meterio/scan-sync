...
export namespace MeterPow {
  // export type VMOutput = Flex.Meter.VMOutput
}

export class MeterPow {
  ...
  public async getBlock<T extends 'expanded' | 'regular'>(
      ...
      if (b) {
        if (new Date().getTime() / 1000 - b.timestamp > 10 * 60) {
          if (expanded) {
            const regular = {
              ...b,
              transactions: (b as Meter.ExpandedBlock).transactions.map(
                (x) => x.id
              ),
            };
    ...
    return cacheOrLoad(() => {
      return this.httpGet<Meter.Block<T> | null>(`blocks/${revision}`, {
        expanded,
      });
    });
  }
  public getTransaction(id: string, head?: string) {
    return this.httpGet<Meter.Transaction>(
      `transactions/${id}`,
      head ? { head } : {}
    );
  }
  public getReceipt(id: string, head?: string) {
    return this.httpGet<Meter.Receipt>(
  ...
  public async getAccount(addr: string, revision?: string) {
    const get = () => {
      return this.httpGet<Meter.Account>(
        `accounts/${addr}`,
        revision ? { revision } : {}
      );
    };
  ...
  public getCode(addr: string, revision?: string) {
    return this.httpGet<Meter.Code>(
      `accounts/${addr}/code`,
      revision ? { revision } : {}
    );
  }
  public getStorage(addr: string, key: string, revision?: string) {
    return this.httpGet<Meter.Storage>(
